...
// In-memory store for tracking processed message IDs to prevent duplicate responses
// This prevents race conditions and duplicate webhook events from causing multiple AI responses
const processedMessages = new Set<string>();
const MESSAGE_PROCESSING_TIMEOUT = 30000; // 30 seconds to prevent memory leaks
...
export async function POST(req: NextRequest) {
  ...
  try {
    if (eventType === "call.session_started") {
      ...
      console.log("[webhook] New message received:", { userId, channelId, text, messageId });
      ...
      // Check if this message is already being processed to prevent duplicate responses
      const messageKey = `${channelId}:${messageId}`;
      if (processedMessages.has(messageKey)) {
        console.log("[webhook] Message already being processed, skipping duplicate", { messageKey });
        return NextResponse.json({ status: "ok" });
      }
      ...
      setTimeout(() => {
        ...
        console.log("[webhook] Cleaned up message tracking for", messageKey);
      ...
      try {
        ...
        // Remove message from processing set on error to allow retry
        processedMessages.delete(messageKey);
        console.log("[webhook] Removed message from tracking due to error", { messageKey });
        // Return success to prevent webhook retries, but log the error
        return NextResponse.json({ status: "error", message: "Message processing failed" });
      }
    }
  } catch (error) {
